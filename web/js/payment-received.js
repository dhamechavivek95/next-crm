(function (factory) {
  ...
  'use strict';
  $.widget("nextcrm.getInvoices", {
    _create: function () {
      var self = this;

      $.proxy(self.initializeVariables, self)();
      $.proxy(self.fetchInvoices, self)();
    ...
    fetchInvoices: function(){
      ...
      $(document).on('change','#paymentdetails-user_id',function(e){
        ...
        $.post('fetch-invoices',{user_id:user_id},function(response){
          self.totalAmount = 0;
          self.dueAmount = 0;
          ...
          var bodyHtml = '';
          if(responseData == ''){
            bodyHtml = "<tr><td>There are no invoices applied for this payment.</td><tr>";
          }
          $.each(responseData,function(index,value){
            bodyHtml += "<tr><td hidden><input name='invoice_id[]' value='"+value.inv_id+"' hidden></td><td class='date'>"+value.created_date+"</td><td class='invoice_number'>"+value.invoice_number+"</td><td class='amount'>"+value.total_amount+"</td><td class='due_amount'>"+value.due_amount+"</td><td class='payment col-sm-2'><input type='text' name='invoice-payment[]' class='payment-input form-control' onkeypress='return isNumberKey(event)'></td></tr>";
...
$(document).ready(function () {
  $("#payment-received-active-form").getInvoices();
});